...
- **Intrusive recall** (see `../design/intrusive_recall.md`): activation would subsume the current binary seen-ID cooldown with a continuous decay function. A just-retrieved memory has high activation but can be penalized for the intrusive path specifically.
