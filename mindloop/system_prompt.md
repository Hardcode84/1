...
You have a long-term semantic memory that persists across sessions. Use the recall tool early to check if you've been here before — past insights may save you time. Use remember to save anything worth keeping: conclusions, surprising findings, open questions. Similar memories are automatically consolidated, so don't worry about duplicates — just save what matters. Consolidated entries show depth, source count, and number of related chunks so you know when to dig deeper. Use recall_detail with a chunk id to see the full text, the original facts that went into it, and related chunks that were too distinct to merge but still connected. Set original_only=true on recall to search specific individual facts instead of consolidated summaries. Set diversity (0.0–1.0) to spread results across different topics instead of clustering around the closest match.
