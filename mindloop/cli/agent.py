...
def _format_pinned_turns(windows: list[dict[str, str]]) -> str:
    """Format pinned turn windows for the system prompt."""
    parts = []
    for i, w in enumerate(windows, 1):
        parts.append(f"## Window {i}: {w['reason']}\n{w['text']}")
    return "\n\n".join(parts)
...
def _build_system_prompt(
    ...
    if paths.workspace and initial_messages is None:
        ...
        pinned_path = paths.root / "_pinned_turns.json" if paths.root else None
        pinned = load_pinned_turns(pinned_path) if pinned_path else None
        if pinned:
            system_prompt += "\n\n# Pinned context from previous session\n"
            system_prompt += _format_pinned_turns(pinned)
    ...
    # Load distilled values (emergent self-understanding from past sessions).
    if paths.workspace:
        values_path = paths.workspace / "_values.md"
        registry.write_blocked[values_path.resolve()] = "autogenerated between sessions"
...
def _distill_session_values(paths: SessionPaths, model: str) -> None:
    """Distill accumulated dispositions into a values statement."""
    if not paths.root or not paths.workspace:
        return
    # Dispositions live in session root (outside workspace sandbox).
    disp_path = paths.root / "_dispositions.jsonl"
    if not disp_path.is_file():
        return
    try:
        values = distill_values(disp_path, model=model)
        if values:
            save_values(paths.workspace / "_values.md", values)
            print("Values distilled to _values.md")
    except Exception as exc:
...
class MidSessionExtractor:
    ...
    Disposition extraction runs in parallel for emergent values.
    """
    ...
    def __init__(
        ...
        self._pinned_windows: list[dict[str, str]] = []
    ...
    def _drain(self, wait: bool = False, timeout: int = 300) -> None:
        ...
        # Drain disposition extraction.
        if self._disp_future is not None:
            if not wait and not self._disp_future.done():
                pass
            else:
                try:
    ...
    def pinned_windows(self) -> list[dict[str, str]]:
        """Return a copy of accumulated pinned windows."""
        return list(self._pinned_windows)

    def critic_review(self) -> str:
        """Fresh-context review of recent agent actions."""
        result = _critic_review(self._last_query, self._model, self._log)
        if result.pin_reason and self._last_query:
            self._pinned_windows.append(
                {
                    "text": self._last_query,
                    "reason": result.pin_reason,
                }
            )
...
def main() -> None:
    ...
    with Executor():
        try:
            ...
            mid_extract.finish()
            if paths.root:
                pinned_path = paths.root / "_pinned_turns.json"
